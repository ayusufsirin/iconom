...
snapshot_artifacts() {
  ...
  docker exec "${ROS2_APP}" bash -lc 'cat /tmp/task-2-slice3-harness-detector.log' \
...
build_and_start_nodes() {
  ...
  docker exec "${ROS2_APP}" bash -lc '
    ...
    nohup ros2 run iconom_vision aircraft_detector >/tmp/task-2-slice3-harness-detector.log 2>&1 &
