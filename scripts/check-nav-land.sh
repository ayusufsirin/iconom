...
PX4_LOG="${ROOT_DIR}/.tmp-px4-nav-land.log"
ARM_LOG="${ROOT_DIR}/.tmp-nav-land-arm.log"
TAKEOFF_LOG="${ROOT_DIR}/.tmp-nav-land-takeoff.log"
LOITER_LOG="${ROOT_DIR}/.tmp-nav-land-loiter.log"
LAND_LOG="${ROOT_DIR}/.tmp-nav-land-command.log"
STATUS_LOG="${ROOT_DIR}/.tmp-nav-land-status.log"
TAKEOFF_POSITION_LOG="${ROOT_DIR}/.tmp-nav-land-takeoff-position.log"
LAND_POSITION_LOG="${ROOT_DIR}/.tmp-nav-land-position.log"
LAND_DETECTED_LOG="${ROOT_DIR}/.tmp-nav-land-detected.log"
PX4_PID=""
...
if [[ "${USE_GUI}" == "1" || "${PX4_HEADLESS:-1}" == "0" ]]; then
  ...
  if [[ -z "${DISPLAY:-}" ]]; then
    echo "DISPLAY is not set; GUI nav land requires a local X11 display" >&2
    exit 229
  fi
...
LAND_DETECTED_TOPIC="${PX4_LAND_DETECTED_TOPIC:-/plane_01/fmu/out/vehicle_land_detected}"
...
echo "land detected topic: ${LAND_DETECTED_TOPIC}"
echo "takeoff altitude offset: ${TAKEOFF_ALT_OFFSET_M}m"
echo "gui mode: ${USE_GUI}"
echo
echo "this checks the phase-3 landing slice:"
echo "  - PX4 runtime starts"
echo "  - ROS arms the aircraft"
echo "  - ROS sends NAV_TAKEOFF to get airborne"
echo "  - ROS enters AUTO_LOITER"
echo "  - ROS sends NAV_LAND"
echo "  - VehicleStatus enters AUTO_LAND"
echo "  - VehicleLocalPosition shows landing descent"
echo "  - VehicleLandDetected reports landed=true"
echo
...
for service in gazebo xrce_agent ros2_app; do
  if ! grep -qx "${service}" <<<"${RUNNING_SERVICES}"; then
    echo "${service} did not reach running state before nav land check" >&2
...
echo "step 7: polling ROS 2 graph for landing topics"
for ((i=1; i<=DISCOVERY_WAIT_SEC; i++)); do
  if [[ -n "${PX4_PID}" ]] && ! kill -0 "${PX4_PID}" >/dev/null 2>&1; then
    PX4_EXIT=0
    wait "${PX4_PID}" || PX4_EXIT=$?
    echo "px4 runtime exited before nav land topic discovery" >&2
  ...
  if grep -qx "${COMMAND_TOPIC}" <<<"${TOPICS}" \
    ...
    && grep -qx "${GLOBAL_POSITION_TOPIC}" <<<"${TOPICS}" \
    && grep -qx "${LAND_DETECTED_TOPIC}" <<<"${TOPICS}"; then
    break
  fi
...
" >"${STATUS_LOG}" 2>&1; then
  echo "VehicleStatus did not report preflight-ready state before nav land" >&2
...
" >"${ARM_LOG}" 2>&1; then
  echo "the arm command did not succeed before nav land validation" >&2
  cat "${ARM_LOG}" >&2 || true
  tail -n 200 "${PX4_LOG}" >&2 || true
  exit 224
fi
...
if ! "${COMPOSE_CMD[@]}" "${COMPOSE_ARGS[@]}" exec -T ros2_app bash -lc "
  ...
  ros2 run iconom_control vehicle_status_waiter
" >"${STATUS_LOG}" 2>&1; then
  echo "VehicleStatus did not report armed state before nav land" >&2
  cat "${STATUS_LOG}" >&2 || true
  tail -n 200 "${PX4_LOG}" >&2 || true
  exit 225
fi
...
" >"${TAKEOFF_LOG}" 2>&1; then
  echo "the NAV_TAKEOFF command did not succeed before landing" >&2
...
if ! "${COMPOSE_CMD[@]}" "${COMPOSE_ARGS[@]}" exec -T ros2_app bash -lc "
  ...
  ros2 run iconom_control vehicle_status_waiter
" >"${STATUS_LOG}" 2>&1; then
  echo "VehicleStatus did not enter AUTO_TAKEOFF before landing" >&2
  cat "${STATUS_LOG}" >&2 || true
  cat "${TAKEOFF_LOG}" >&2 || true
  tail -n 200 "${PX4_LOG}" >&2 || true
  exit 227
...
" >"${TAKEOFF_POSITION_LOG}" 2>&1; then
  echo "VehicleLocalPosition did not show takeoff motion before landing" >&2
...
if ! "${COMPOSE_CMD[@]}" "${COMPOSE_ARGS[@]}" exec -T ros2_app bash -lc "
  ...
  ros2 run iconom_control vehicle_command_client
" >"${LOITER_LOG}" 2>&1; then
  echo "the mode_loiter command did not succeed before landing" >&2
...
if ! "${COMPOSE_CMD[@]}" "${COMPOSE_ARGS[@]}" exec -T ros2_app bash -lc "
  ...
  ros2 run iconom_control vehicle_status_waiter
" >"${STATUS_LOG}" 2>&1; then
  echo "VehicleStatus did not enter AUTO_LOITER before landing" >&2
  cat "${STATUS_LOG}" >&2 || true
  cat "${LOITER_LOG}" >&2 || true
  tail -n 200 "${PX4_LOG}" >&2 || true
  exit 231
...
echo "step 16: sending NAV_LAND through the navigation client"
if ! "${COMPOSE_CMD[@]}" "${COMPOSE_ARGS[@]}" exec -T ros2_app bash -lc "
  set -euo pipefail
  set +u
  source /opt/ros/humble/setup.bash
  ...
  export PX4_COMMAND_ACK_TOPIC='${COMMAND_ACK_TOPIC}'
  export PX4_GLOBAL_POSITION_TOPIC='${GLOBAL_POSITION_TOPIC}'
  export PX4_NAV_COMMAND_NAME='nav_land'
  export PX4_TARGET_OFFSET_ALT_M='0.0'
  export PX4_COMMAND_TIMEOUT_SEC='${PX4_COMMAND_TIMEOUT_SEC:-20}'
  ros2 run iconom_control navigation_command_client
" >"${LAND_LOG}" 2>&1; then
  echo "the NAV_LAND command did not succeed" >&2
  cat "${LAND_LOG}" >&2 || true
  tail -n 200 "${PX4_LOG}" >&2 || true
  exit 232
fi

echo "step 17: waiting for VehicleStatus.nav_state=AUTO_LAND"
if ! "${COMPOSE_CMD[@]}" "${COMPOSE_ARGS[@]}" exec -T ros2_app bash -lc "
  set -euo pipefail
  set +u
  source /opt/ros/humble/setup.bash
  ...
  export PX4_VEHICLE_STATUS_TOPIC='${STATUS_TOPIC}'
  export PX4_STATUS_TIMEOUT_SEC='${STATUS_TIMEOUT_SEC}'
  export PX4_EXPECTED_NAV_STATE='${LAND_NAV_STATE}'
  ros2 run iconom_control vehicle_status_waiter
" >"${STATUS_LOG}" 2>&1; then
  echo "VehicleStatus did not enter AUTO_LAND after NAV_LAND" >&2
  cat "${STATUS_LOG}" >&2 || true
  cat "${LAND_LOG}" >&2 || true
  tail -n 200 "${PX4_LOG}" >&2 || true
  exit 233
...
echo "step 18: waiting for landing descent in VehicleLocalPosition"
if ! "${COMPOSE_CMD[@]}" "${COMPOSE_ARGS[@]}" exec -T ros2_app bash -lc "
  set -euo pipefail
  set +u
  source /opt/ros/humble/setup.bash
  ...
  export ICONOM_VEHICLE_NAMESPACE='${ICONOM_VEHICLE_NAMESPACE}'
  export PX4_LOCAL_POSITION_TOPIC='${LOCAL_POSITION_TOPIC}'
  export PX4_LOCAL_POSITION_TIMEOUT_SEC='${PX4_LOCAL_POSITION_TIMEOUT_SEC:-120}'
  export PX4_MIN_DELTA_Z='${LAND_MIN_DELTA_Z}'
  ros2 run iconom_control vehicle_local_position_waiter
" >"${LAND_POSITION_LOG}" 2>&1; then
  echo "VehicleLocalPosition did not show landing descent after NAV_LAND" >&2
...
echo "step 19: waiting for VehicleLandDetected.landed=true"
if ! "${COMPOSE_CMD[@]}" "${COMPOSE_ARGS[@]}" exec -T ros2_app bash -lc "
  set -euo pipefail
  set +u
  source /opt/ros/humble/setup.bash
  ...
  export PX4_LAND_DETECTED_TIMEOUT_SEC='${LAND_DETECTED_TIMEOUT_SEC}'
  export PX4_EXPECTED_LANDED='true'
  ros2 run iconom_control vehicle_land_detected_waiter
" >"${LAND_DETECTED_LOG}" 2>&1; then
  echo "VehicleLandDetected did not report landed=true after NAV_LAND" >&2
...
echo "nav land guidance is alive"
cat "${TAKEOFF_LOG}"
