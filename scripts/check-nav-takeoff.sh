...
PX4_LOG="${ROOT_DIR}/.tmp-px4-nav-takeoff.log"
ARM_LOG="${ROOT_DIR}/.tmp-nav-takeoff-arm.log"
NAV_LOG="${ROOT_DIR}/.tmp-nav-takeoff-command.log"
STATUS_LOG="${ROOT_DIR}/.tmp-nav-takeoff-status.log"
POSITION_LOG="${ROOT_DIR}/.tmp-nav-takeoff-position.log"
PX4_PID=""
...
cleanup() {
  ...
  rm -f "${PX4_LOG}" "${ARM_LOG}" "${NAV_LOG}" "${STATUS_LOG}" "${POSITION_LOG}"
...
if [[ "${USE_GUI}" == "1" || "${PX4_HEADLESS:-1}" == "0" ]]; then
  ...
  if [[ -z "${DISPLAY:-}" ]]; then
    echo "DISPLAY is not set; GUI nav takeoff requires a local X11 display" >&2
    exit 149
  fi
...
EXPECTED_NAV_STATE="${PX4_EXPECTED_TAKEOFF_NAV_STATE:-17}"
MIN_DELTA_XY_NORM="${PX4_EXPECTED_TAKEOFF_MIN_DELTA_XY_NORM:-5.0}"
MAX_DELTA_Z="${PX4_EXPECTED_TAKEOFF_MAX_DELTA_Z:--0.5}"

echo "iconom nav takeoff check"
...
echo "target altitude offset: ${TAKEOFF_ALT_OFFSET_M}m"
echo "gui mode: ${USE_GUI}"
echo
echo "this checks the first PX4-native guidance slice:"
echo "  - PX4 runtime starts"
echo "  - ROS arms the aircraft"
echo "  - ROS sends NAV_TAKEOFF through VehicleCommand"
echo "  - VehicleStatus enters AUTO_TAKEOFF"
echo "  - VehicleLocalPosition shows takeoff motion"
echo
...
for service in gazebo xrce_agent ros2_app; do
  if ! grep -qx "${service}" <<<"${RUNNING_SERVICES}"; then
    echo "${service} did not reach running state before nav takeoff check" >&2
...
for ((i=1; i<=DISCOVERY_WAIT_SEC; i++)); do
  if [[ -n "${PX4_PID}" ]] && ! kill -0 "${PX4_PID}" >/dev/null 2>&1; then
    ...
    echo "px4 runtime exited before nav takeoff topic discovery" >&2
...
" >"${STATUS_LOG}" 2>&1; then
  echo "VehicleStatus did not report preflight-ready state before nav takeoff" >&2
...
" >"${ARM_LOG}" 2>&1; then
  echo "the arm command did not succeed before nav takeoff validation" >&2
  cat "${ARM_LOG}" >&2 || true
  tail -n 200 "${PX4_LOG}" >&2 || true
  exit 144
fi
...
if ! "${COMPOSE_CMD[@]}" "${COMPOSE_ARGS[@]}" exec -T ros2_app bash -lc "
  ...
  ros2 run iconom_control vehicle_status_waiter
" >"${STATUS_LOG}" 2>&1; then
  echo "VehicleStatus did not report armed state before nav takeoff" >&2
  cat "${STATUS_LOG}" >&2 || true
  tail -n 200 "${PX4_LOG}" >&2 || true
  exit 145
fi

echo "step 11: sending NAV_TAKEOFF through the new navigation client"
if ! "${COMPOSE_CMD[@]}" "${COMPOSE_ARGS[@]}" exec -T ros2_app bash -lc "
  set -euo pipefail
  set +u
  source /opt/ros/humble/setup.bash
...
" >"${NAV_LOG}" 2>&1; then
  echo "the NAV_TAKEOFF command did not succeed" >&2
  cat "${NAV_LOG}" >&2 || true
  tail -n 200 "${PX4_LOG}" >&2 || true
  exit 146
fi
...
if ! "${COMPOSE_CMD[@]}" "${COMPOSE_ARGS[@]}" exec -T ros2_app bash -lc "
  ...
  ros2 run iconom_control vehicle_status_waiter
" >"${STATUS_LOG}" 2>&1; then
  echo "VehicleStatus did not enter AUTO_TAKEOFF after NAV_TAKEOFF" >&2
  cat "${STATUS_LOG}" >&2 || true
  cat "${NAV_LOG}" >&2 || true
  tail -n 200 "${PX4_LOG}" >&2 || true
  exit 147
...
" >"${POSITION_LOG}" 2>&1; then
  echo "VehicleLocalPosition did not show takeoff motion after NAV_TAKEOFF" >&2
...
echo "nav takeoff guidance is alive"
cat "${NAV_LOG}"
cat "${POSITION_LOG}"
