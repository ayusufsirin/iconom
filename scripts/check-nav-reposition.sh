...
PX4_LOG="${ROOT_DIR}/.tmp-px4-nav-reposition.log"
ARM_LOG="${ROOT_DIR}/.tmp-nav-reposition-arm.log"
TAKEOFF_LOG="${ROOT_DIR}/.tmp-nav-reposition-takeoff.log"
LOITER_LOG="${ROOT_DIR}/.tmp-nav-reposition-loiter.log"
REPOSITION_LOG="${ROOT_DIR}/.tmp-nav-reposition-command.log"
STATUS_LOG="${ROOT_DIR}/.tmp-nav-reposition-status.log"
TAKEOFF_POSITION_LOG="${ROOT_DIR}/.tmp-nav-reposition-takeoff-position.log"
REPOSITION_TARGET_LOG="${ROOT_DIR}/.tmp-nav-reposition-target.log"
PX4_PID=""
...
if [[ "${USE_GUI}" == "1" || "${PX4_HEADLESS:-1}" == "0" ]]; then
  ...
  if [[ -z "${DISPLAY:-}" ]]; then
    echo "DISPLAY is not set; GUI nav reposition requires a local X11 display" >&2
    exit 189
  fi
...
echo "iconom nav reposition check"
...
echo "reposition north/east/alt offsets: ${REPOSITION_OFFSET_NORTH_M}m ${REPOSITION_OFFSET_EAST_M}m ${REPOSITION_OFFSET_ALT_M}m"
...
echo "  - ROS sends DO_REPOSITION with a bounded global offset"
echo "  - VehicleStatus remains AUTO_LOITER"
echo "  - VehicleGlobalPosition approaches the reposition target"
echo
...
for service in gazebo xrce_agent ros2_app; do
  if ! grep -qx "${service}" <<<"${RUNNING_SERVICES}"; then
    echo "${service} did not reach running state before nav reposition check" >&2
...
for ((i=1; i<=DISCOVERY_WAIT_SEC; i++)); do
  if [[ -n "${PX4_PID}" ]] && ! kill -0 "${PX4_PID}" >/dev/null 2>&1; then
    ...
    echo "px4 runtime exited before nav reposition topic discovery" >&2
...
" >"${STATUS_LOG}" 2>&1; then
  echo "VehicleStatus did not report preflight-ready state before nav reposition" >&2
...
" >"${ARM_LOG}" 2>&1; then
  echo "the arm command did not succeed before nav reposition validation" >&2
  cat "${ARM_LOG}" >&2 || true
  tail -n 200 "${PX4_LOG}" >&2 || true
  exit 184
fi
...
if ! "${COMPOSE_CMD[@]}" "${COMPOSE_ARGS[@]}" exec -T ros2_app bash -lc "
  ...
  ros2 run iconom_control vehicle_status_waiter
" >"${STATUS_LOG}" 2>&1; then
  echo "VehicleStatus did not report armed state before nav reposition" >&2
  cat "${STATUS_LOG}" >&2 || true
  tail -n 200 "${PX4_LOG}" >&2 || true
  exit 185
fi
...
" >"${TAKEOFF_LOG}" 2>&1; then
  echo "the NAV_TAKEOFF command did not succeed before reposition" >&2
...
if ! "${COMPOSE_CMD[@]}" "${COMPOSE_ARGS[@]}" exec -T ros2_app bash -lc "
  ...
  ros2 run iconom_control vehicle_status_waiter
" >"${STATUS_LOG}" 2>&1; then
  echo "VehicleStatus did not enter AUTO_TAKEOFF before reposition" >&2
  cat "${STATUS_LOG}" >&2 || true
  cat "${TAKEOFF_LOG}" >&2 || true
  tail -n 200 "${PX4_LOG}" >&2 || true
  exit 187
...
" >"${TAKEOFF_POSITION_LOG}" 2>&1; then
  echo "VehicleLocalPosition did not show takeoff motion before reposition" >&2
...
if ! "${COMPOSE_CMD[@]}" "${COMPOSE_ARGS[@]}" exec -T ros2_app bash -lc "
  ...
  ros2 run iconom_control vehicle_command_client
" >"${LOITER_LOG}" 2>&1; then
  echo "the mode_loiter command did not succeed before reposition" >&2
...
if ! "${COMPOSE_CMD[@]}" "${COMPOSE_ARGS[@]}" exec -T ros2_app bash -lc "
  ...
  ros2 run iconom_control vehicle_status_waiter
" >"${STATUS_LOG}" 2>&1; then
  echo "VehicleStatus did not enter AUTO_LOITER before reposition" >&2
  cat "${STATUS_LOG}" >&2 || true
  cat "${LOITER_LOG}" >&2 || true
  tail -n 200 "${PX4_LOG}" >&2 || true
  exit 191
...
if ! "${COMPOSE_CMD[@]}" "${COMPOSE_ARGS[@]}" exec -T ros2_app bash -lc "
  ...
  ros2 run iconom_control vehicle_local_position_waiter
" >"${STATUS_LOG}" 2>&1; then
  echo "VehicleLocalPosition did not show active loiter motion before reposition" >&2
  cat "${STATUS_LOG}" >&2 || true
  cat "${LOITER_LOG}" >&2 || true
  tail -n 200 "${PX4_LOG}" >&2 || true
  exit 192
...
echo "step 17: sending DO_REPOSITION through the navigation client"
if ! "${COMPOSE_CMD[@]}" "${COMPOSE_ARGS[@]}" exec -T ros2_app bash -lc "
  set -euo pipefail
  set +u
  source /opt/ros/humble/setup.bash
  ...
  export PX4_TARGET_OFFSET_NORTH_M='${REPOSITION_OFFSET_NORTH_M}'
  ...
  export PX4_COMMAND_TIMEOUT_SEC='${PX4_COMMAND_TIMEOUT_SEC:-20}'
  ros2 run iconom_control navigation_command_client
" >"${REPOSITION_LOG}" 2>&1; then
  echo "the DO_REPOSITION command did not succeed" >&2
  cat "${REPOSITION_LOG}" >&2 || true
  tail -n 200 "${PX4_LOG}" >&2 || true
  exit 193
fi

TARGET_FIELDS="$(grep -m1 'toward lat=' "${REPOSITION_LOG}" | sed -E 's/.*toward lat=([^ ]+) lon=([^ ]+) alt=([^ ]+).*/\1 \2 \3/')"
if [[ -z "${TARGET_FIELDS}" ]]; then
  echo "could not extract the reposition target from the navigation log" >&2
  cat "${REPOSITION_LOG}" >&2 || true
  exit 194
fi
read -r TARGET_LAT_DEG TARGET_LON_DEG TARGET_ALT_M <<<"${TARGET_FIELDS}"

echo "step 18: confirming VehicleStatus remains AUTO_LOITER"
if ! "${COMPOSE_CMD[@]}" "${COMPOSE_ARGS[@]}" exec -T ros2_app bash -lc "
  set -euo pipefail
  set +u
  source /opt/ros/humble/setup.bash
  ...
  ros2 run iconom_control vehicle_status_waiter
" >"${STATUS_LOG}" 2>&1; then
  echo "VehicleStatus did not remain AUTO_LOITER after DO_REPOSITION" >&2
  cat "${STATUS_LOG}" >&2 || true
  cat "${REPOSITION_LOG}" >&2 || true
  tail -n 200 "${PX4_LOG}" >&2 || true
  exit 195
...
echo "step 19: waiting for VehicleGlobalPosition to approach the reposition target"
if ! "${COMPOSE_CMD[@]}" "${COMPOSE_ARGS[@]}" exec -T ros2_app bash -lc "
  set -euo pipefail
  set +u
  source /opt/ros/humble/setup.bash
...
" >"${REPOSITION_TARGET_LOG}" 2>&1; then
  echo "VehicleGlobalPosition did not approach the reposition target" >&2
  cat "${REPOSITION_TARGET_LOG}" >&2 || true
  cat "${REPOSITION_LOG}" >&2 || true
  tail -n 200 "${PX4_LOG}" >&2 || true
  exit 196
fi

echo "nav reposition guidance is alive"
cat "${TAKEOFF_LOG}"
cat "${TAKEOFF_POSITION_LOG}"
cat "${LOITER_LOG}"
cat "${REPOSITION_LOG}"
cat "${REPOSITION_TARGET_LOG}"
