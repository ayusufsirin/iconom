...
PREDICTOR_PID=$!
ros2_exec "set -euo pipefail; set +u; source /opt/ros/humble/setup.bash; source /workspaces/ros2_ws/install/setup.bash; set -u; /workspaces/ros2_ws/install/bin/target_selector --ros-args -p publish_period_sec:=\"${SELECTOR_PERIOD_SEC}\"" >"${SELECTOR_LOG}" 2>&1 &
SELECTOR_PID=$!
ros2_exec "set -euo pipefail; set +u; source /opt/ros/humble/setup.bash; source /workspaces/ros2_ws/install/setup.bash; set -u; /workspaces/ros2_ws/install/bin/intercept_planner --ros-args -p max_intercept_distance:=80.0 -p publish_period_sec:=\"${PLANNER_PERIOD_SEC}\"" >"${PLANNER_LOG}" 2>&1 &
PLANNER_PID=$!
